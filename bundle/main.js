        ...
        for (const path2 of optionPaths) {
            try {
                const parsed = DotenvModule.parse(
                    fs.readFileSync(path2, { encoding }),
                );
        ...
        for (const key of Object.keys(parsed)) {
            if (Object.prototype.hasOwnProperty.call(processEnv, key)) {
                ...
                if (debug) {
                    if (override === true) {
                        _debug(
                            `"${key}" is already defined and WAS overwritten`,
                        );
                    } else {
                        _debug(
        ...
        _executeSubCommand(subcommand, args) {
            ...
            if (process2.platform !== "win32") {
                if (launchWithNode) {
                    ...
                    args = incrementNodeInspectorPort(process2.execArgv).concat(
                        args,
                    );
            ...
            this.options
                ...
                .forEach((option) => {
                    Object.keys(option.implied)
                        .filter(
                            (impliedKey) => !hasCustomOptionValue(impliedKey),
                        )
                        .forEach((impliedKey) => {
                            this.setOptionValueWithSource(
                                impliedKey,
...
`);
        ...
        executableDir(path4) {
            if (path4 === undefined) return this._executableDir;
...
`);
    ...
    function incrementNodeInspectorPort(args) {
        return args.map((arg) => {
            ...
            if ((match = arg.match(/^(--inspect(-brk)?)$/)) !== null) {
                ...
                (match = arg.match(/^(--inspect(-brk|-port)?)=([^:]+)$/)) !==
                null
            ) {
                debugOption = match[1];
                if (/^\d+$/.test(match[3])) {
    ...
    function useColor() {
        if (
            ...
            return false;
        if (
            process2.env.FORCE_COLOR ||
            process2.env.CLICOLOR_FORCE !== undefined
        )
            return true;
        return;
    }
...
var require_process = __commonJS((exports, module) => {
    ...
    var ProcessErrorCatch = class {
        ...
        constructor() {
            ...
            this.unhandledRejectionRegister =
                this.unhandledRejectionRegister.bind(this);
        }
        unExceptionRegister(...args) {
            if (this.cb) this.cb("uncaughtException", ...args);
        }
        unhandledRejectionRegister(...args) {
            if (this.cb) this.cb("unhandledRejection", ...args);
        }
        listen(cb) {
            if (!process) {
...
var config_tool_file_path = () =>
    path2.resolve(config_cache_dir(), "config.ini");
var config_pkg_manger_file_path = () =>
    path2.resolve(config_cache_dir(), "pkg_manager.ini");
var config_tl_file_path = () => path2.resolve(config_cache_dir(), "tl.ini");
var config_claude_file_path = () =>
    path2.resolve(config_cache_dir(), "claude_model.ini");
        ...
        for (var index = 0; index < items.length; ++index) {
            ...
            for (var key in item) {
                ...
                result[key] = isRecursive
                    ? _recursiveMerge(result[key], value)
                    : value;
            }
        }
        return result;
...
var createFile = (targetPath, content, cover = false) => {
    ...
    if (!fs2.existsSync(dirPathName)) createDir(dirPathName);
...
var parentExecHandlerPromise = (targetPath, cb) => {
    return new Promise((resolve2) => {
        if (isDriveDirectory(targetPath)) return resolve2("");
        const recursionExecHandler = () =>
            resolve2(parentExecHandlerPromise(path3.dirname(targetPath), cb));
        if (isType(cb, "asyncfunction")) {
            ...
            try {
                ...
                if (result) return resolve2(result);
...
var checkSystem = async () => {
    ...
    if (!nodeVer && !bunVer) {
        throw new Error(
            "\u672A\u68C0\u6D4B\u5230Node\u4E14Bun\u73AF\u5883\uFF0C\u81F3\u5C11\u5B89\u88C5\u4E00\u4E2A\uFF1ANode\u3001Bun",
        );
    }
...
class Config {
    ...
    #load() {
        createFile(
            this.#sourcePath,
            import_ini.default.stringify(this.#defaultContent).trim(),
        );
        this.#resetOptionConfig(
            import_ini.default.parse(
                readExistsFile(this.#sourcePath).toString(),
            ),
        );
    ...
    reset() {
        createFile(
            ...
            import_ini.default.stringify(Object.fromEntries(this.data)).trim(),
            true,
        );
...
var defaultTheme = {
    ...
    spinner: {
        ...
        frames: [
            ...
            "\u2807",
            "\u280F",
        ].map((frame) => styleText("yellow", frame)),
    },
    style: {
...
function validate(codePoint) {
    if (!Number.isSafeInteger(codePoint)) {
        throw new TypeError(
            `Expected a code point, got \`${typeof codePoint}\`.`,
        );
    }
}
...
function readlineWidth() {
    return import_cli_width.default({
        defaultWidth: 80,
            ...
            if (
                ...
                pointer = Math.min(
                    ...
                    Math.abs(active - lastActive) === 1
                        ? Math.min(
                              lastPointer +
                                  (renderedItems[lastActive]?.length ?? 0),
...
class Separator {
    separator = styleText2(
        "dim",
        Array.from({ length: 15 }).join(dist_default.line),
    );
    type = "separator";
    ...
    style: {
        ...
        keysHelpTip: (keys) =>
            keys
                .map(
                    ([key, action]) =>
                        `${styleText3("bold", key)} ${styleText3("dim", action)}`,
                )
                .join(styleText3("dim", " \u2022 ")),
    },
    keybindings: [],
...
var dist_default3 = createPrompt((config, done) => {
    ...
    useKeypress((key, rl) => {
        ...
        if (isEnterKey(key)) {
            ...
            const answer = boolToString(
                !getBooleanValue(value, config.default),
            );
            rl.clearLine(0);
            rl.write(answer);
            setValue(answer);
    ...
    style: {
        ...
        keysHelpTip: (keys) =>
            keys
                .map(
                    ([key, action]) =>
                        `${styleText4("bold", key)} ${styleText4("dim", action)}`,
                )
                .join(styleText4("dim", " \u2022 ")),
    },
};
    ...
    style: {
        ...
        keysHelpTip: (keys) =>
            keys
                .map(
                    ([key, action]) =>
                        `${styleText5("bold", key)} ${styleText5("dim", action)}`,
                )
                .join(styleText5("dim", " \u2022 ")),
    },
    indexMode: "hidden",
...
var dist_default6 = createPrompt((config, done) => {
    ...
    const items = useMemo(
        () => normalizeChoices3(config.choices),
        [config.choices],
    );
    const bounds = useMemo(() => {
    ...
    const defaultItemIndex = useMemo(() => {
        ...
        return items.findIndex(
            (item) => isSelectable3(item) && item.value === config.default,
        );
        ...
        return arrayExecSyncHandler((item) => {
            ...
            if (defaultValue) {
                const existsItem = dataSource.findIndex(
                    (item2) => item2.value === defaultValue,
                );
                if (existsItem !== -1) {
            ...
            return dist_default5({
                ...
                source: async (input) => {
                    return input?.trim?.()
                        ? dataSource.filter((item2) =>
                              item2.name.includes(input),
                          )
                        : dataSource;
                },
            });
...
var qsForAskInitStorage = () => {
    return {
        ...
        require: true,
        message:
            "\u5F53\u524D\u76EE\u5F55\u4E0D\u662F\u4E00\u4E2Agit\u5DE5\u7A0B\u76EE\u5F55\uFF0C\u662F\u5426\u9700\u8981\u5728\u6B64\u76EE\u5F55\u4E0B\u521D\u59CB\u5316git\uFF1F",
    };
};
...
`);
        for (const config2 of stringifyData) {
            ...
            if (key in config_default_option) {
                console.log(
                    `${config2.replace("\r", "")} \u3010${config_default_option[key]["description"]}\u3011`,
                );
            }
        }
    }
...
class ExecFile {
    ...
    async verifyFileName(suffix) {
        ...
        if (!support_exec_file.includes(suffix)) {
            throw new Error(
                `\u5F53\u524D\u6587\u4EF6\u4E0D\u652F\u6301\u88AB\u6267\u884C\uFF0C\u4EC5\u652F\u6301 "${support_exec_file.join("/")}" \u6587\u4EF6`,
            );
        }
    }
...
class PjGit extends Git {
        ...
        if (remoteList.length <= 0) {
            ...
            if (remoteList.length > 1) {
                remote = await this.#inquirer.handler(
                    qsForChooseRemote(remoteList.map((it) => it.origin)),
                );
            } else {
    ...
    async chooseStorageSingleRemote() {
        ...
        return await this.#inquirer.handler(
            qsForChooseRemote(remotes.map((it) => it.origin)),
        );
    }
    async confirmBranch() {
        ...
        if (cacheConfig) {
            ...
            for (const ml of claudeModel) {
                ...
                for (const key in cacheConfig) {
                    text.push(
                        `${ml.option[key].text}\uFF1A${cacheConfig[key]}`,
                    );
                }
            }
            console.log(`
...
`);
    ...
    async writeConfig(modelName, option) {
        ...
        if (exists(configPath)) {
            copyFile(
                configPath,
                resolve4(dirname(configPath), `settings-back.json`),
                true,
            );
        }
