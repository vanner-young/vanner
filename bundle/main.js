...
var promptForFilePath = (options = {}) => {
  ...
  const allowedExtensions = Array.isArray(extension) ? extension.map(normalizeExt) : extension ? [normalizeExt(extension)] : [];
  const extDisplay = allowedExtensions.length > 0 ? allowedExtensions.length === 1 ? allowedExtensions[0] : allowedExtensions.join("\u3001") : "";
  const defaultMsg = extDisplay ? multiple ? `\u8BF7\u9009\u62E9\u4E00\u4E2A\u6216\u591A\u4E2A ${extDisplay} \u6587\u4EF6` : `\u8BF7\u9009\u62E9\u4E00\u4E2A ${extDisplay} \u6587\u4EF6` : multiple ? "\u8BF7\u9009\u62E9\u4E00\u4E2A\u6216\u591A\u4E2A\u6587\u4EF6" : "\u8BF7\u9009\u62E9\u4E00\u4E2A\u6587\u4EF6";
...
${displayMsg}`);
    ...
    console.log(multiple ? "  2. \u591A\u4E2A\u6587\u4EF6\u53EF\u7528\u7A7A\u683C\u5206\u9694\uFF0C\u6216\u7EE7\u7EED\u62D6\u62FD/\u8F93\u5165\u4E0B\u4E00\u4E2A\u8DEF\u5F84" : "  2. \u76F4\u63A5\u7C98\u8D34\u6587\u4EF6\u8DEF\u5F84");
    console.log("  3. \u624B\u52A8\u8F93\u5165\u6587\u4EF6\u8DEF\u5F84");
    console.log("");
    const rlPrompt = () => {
      rl.question(`\u6587\u4EF6\u8DEF\u5F84${multiple ? "\uFF08\u53EF\u8F93\u5165\u591A\u4E2A\uFF0C\u7A7A\u683C\u5206\u9694\uFF09" : ""}\uFF08\u6309 Ctrl+C \u9000\u51FA\uFF09: `, (line) => {
...
`);
        ...
        const cleanedPaths = rawPaths.map((path5) => {
          let cleaned = path5.replace(/^["']/, "").replace(/["']$/, "");
          if (process.platform === "win32") {
            cleaned = cleaned.replace(/\\/g, "/");
          }
          return cleaned;
        }).filter((path5) => path5?.trim());
...
`);
        ...
        if (allowedExtensions.length > 0) {
          const invalidPaths = [];
          for (const path5 of cleanedPaths) {
            const fileExt = getFileExtension(path5);
            const normalizedExt = fileExt ? `.${fileExt}` : "";
            if (!allowedExtensions.includes(normalizedExt)) {
              invalidPaths.push(path5);
            }
          }
          if (invalidPaths.length > 0) {
            console.log(`\u9519\u8BEF\uFF1A\u4EE5\u4E0B\u6587\u4EF6\u4E0D\u662F ${extDisplay} \u683C\u5F0F\uFF1A
  ${invalidPaths.join(`
  `)}
`);
            rlPrompt();
...
var isValidGitUrl = (url) => {
  ...
  const gitUrlPattern = /^(https?:\/\/|git@|ssh:\/\/)[^\s/$.?#].[^\s]*?(?:\.git)?$/i;
...
class PjGit extends Git {
  ...
  async confirmPushType() {
    ...
    let { type = "", msg = "" } = await this.#inquirer.handler(qsForPushType(types));
    if (!msg?.trim?.()) {
      while (true) {
        console.log("\u672C\u5730\u63D0\u4EA4\u63CF\u8FF0\u4E0D\u53EF\u4E3A\u7A7A\uFF0C\u8BF7\u91CD\u8BD5~");
        const content = await this.#inquirer.handler(qsForPushType(types).slice(1));
        if (content?.msg?.trim?.()) {
          msg = content.msg;
          break;
        }
...
// packages/core/src/module/claude.ts
import { homedir } from "os";
import { resolve as resolve5, dirname } from "path";
class Claude extends Inquirer {
  configPath = resolve5(homedir(), ".claude", "settings.json");
  backConfigPath = resolve5(dirname(this.configPath), `settings-back.json`);
  config = new Config({
    sourcePath: config_claude_file_path(),
    defaultContent: {}
  });
  async writeConfig(modelName, option, tipsCache = true) {
    if (exists(this.configPath)) {
      copyFile(this.configPath, this.backConfigPath, true);
    }
    const content = { env: option };
    createFile(this.configPath, JSON.stringify(content, undefined, 2), true);
    if (!tipsCache)
      return;
...
`);
  ...
  async switch(modelName = "") {
    if (!modelName) {
      modelName = await this.handler(qsForChooseClaudeModel(claude_model));
    }
  ...
  async import() {
    ...
    let config2 = __require(filePath);
    config2 = config2?.env || {};
    const configKey = [
      "ANTHROPIC_BASE_URL",
      "ANTHROPIC_AUTH_TOKEN",
    ...
    const notExistsKey = configKey.find((key) => !config2[key]);
    if (notExistsKey) {
      throw new Error(`\u914D\u7F6E\u6587\u4EF6\u5185\u5BB9\u65E0\u6548\uFF0C${configKey.join("\u3001")}\u503C\u5FC5\u987B\u5B58\u5728~`);
    }
    await this.writeConfig(config2["ANTHROPIC_MODEL"], config2, false);
    console.log(`${config2["ANTHROPIC_MODEL"]} \u6A21\u578B\u5BFC\u5165\u6210\u529F\uFF0C\u8BF7\u5728\u65B0\u6253\u5F00\u7684\u7EC8\u7AEF\u4E2D\u9A8C\u8BC1~`);
    this.openClaude();
  }
  async start(type) {
      ...
      children: [
        {
          ...
          action: () => {
            new Claude2().start("switch" /* switch */);
          }
        },
        {
          command: "import",
          ...
          action: () => {
            new Claude2().start("import" /* import */);
          }
        }
      ]
    },
