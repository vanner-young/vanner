...
var require$$0$4 = require('node:async_hooks');
var require$$0$5 = require('node:tty');
var require$$0$6 = require('node:process');
var require$$0$7 = require('tty');
var require$$0$8 = require('node:readline');
...
function requireCommon () {
	...
	(function (exports) {
		...
		const isType = (value, type) => {
		    ...
		    const typeValue = typeResult.slice(typeResult.indexOf(' ') + 1, typeResult.length - 1);
		    return typeValue.toLowerCase() === type.toLocaleLowerCase();
		};
		/**
		 * 是否是一个空对象
		 * **/
		...
		/**
		 * 判断一个数据或一组数据是否Array类型
		 * @param { Array } rest 需要判断的数据
		 * **/
		const isArray = (...rest) => {
		    return !rest.find((item) => !Array.isArray(item));
		};
		/**
		 * 扁平化一个JSON对象
		 * @param { Object } 需要扁平化的对象
		...
		const flatObject = (obj) => {
		    ...
		    for (const key in obj) {
		        ...
		        if (typeof item === 'object') {
		            value = merge.recursive(true, value, flatObject(item));
		        }
		        else {
		            value[key] = item;
		        }
		 ...
		 * @param { Object } JSON 内容
		 * @param { String } 分隔符字符串
		 * @return { String } 处理完成之后的字符串
		 * **/
		const handlerJsonContent = (content, sep = ' ') => {
		    let fileContent = '';
		...
		/**
		 * 交集方法
		 * @param { Array<unknown> } list  求交集的数组
		 * @param { Array<unknown> } newList 求交集的数组
		 * @returns { Array<unknown> }
		 * **/
		const intersectionArrayList = (list, newList) => {
		    return list.filter((item) => newList.includes(item));
		};
		/**
		 * 差集方法
		 * @param { Array<unknown> } list  求差集的数组
		 * @param { Array<unknown> } newList 求差集的数组
		 * @returns { Array<unknown> }
		 * **/
		const differenceArrayList = (list, newList) => {
		    return list.filter((item) => !newList.includes(item));
		};
		/**
		 * 并集方法
		 * @param { Array<Array<unknown>> } list  求并集的数组
		 * @returns { Array<unknown> }
		 * **/
		const unionArrayList = (...list) => {
		    return [...new Set(list.flat(1))];
		};
		 ...
		 * **/
		const existsFileForCwd = (filename) => {
		    return fs.existsSync(path.resolve(process.cwd(), filename));
		};
		/**
		 * 读取一个目录下的文件或者文件夹
		 * **/
		const readDirPathTypeFile = (targetPath, type = 'dir') => {
		    if (!fs.existsSync(targetPath))
		        return [];
		    return fs.readdirSync(targetPath).filter((item) => {
		        const itemPath = path.resolve(targetPath, item);
		        const stat = fs.statSync(itemPath).isDirectory();
		        return type === 'file' ? !stat : stat;
		    });
		};
		/**
		 * 当目录存在时，覆盖创建
		 * @param { string } filePath 目标文件目录
		 * **/
		const createCoverDir = (filePath) => {
		 ...
		 * **/
		const writeCoverFile = (filePath, content) => {
		    if (fs.existsSync(filePath))
		...
		/**
		 * 当文件存在时，读取一个文件的文件内容
		 * @param { string } filePath 目标文件路径的
		 * **/
		const readFileIsExists = (filePath, options) => {
		    if (!fs.existsSync(filePath))
		        return '';
		...
		/**
		 * 当文件夹不存在时，创建一个文件目录
		 * @param { string } dirName 目标目录路径
		 * **/
		const createNotExistsFolder = (dirName) => {
		    if (fs.existsSync(dirName))
		        return;
		    return fs.mkdirSync(dirName, { recursive: true });
		};
		/**
		 * 当文件不存在时，创建一个文件并写入文件内容
		 * @param { string } filePath 目标文件路径
		 * @param { string } content 文件的内容
		 * **/
		const writeNotExistsFile = (filePath, content) => {
		    if (fs.existsSync(filePath))
		        return;
		    const fileDirPath = path.dirname(filePath);
		    if (!fs.existsSync(fileDirPath))
		        createNotExistsFolder(fileDirPath);
		 ...
		 * **/
		const copyFile = (sourcePath, destPath, globalCover) => {
		    if (!fs.existsSync(destPath) || globalCover) {
		        console.log('move copy file is:', sourcePath, destPath);
		        return fs.copyFileSync(sourcePath, destPath);
		    }
		    console.log('ignore move copy file:', sourcePath, destPath);
		};
		/**
		 * 拷贝文件至指定的目录
		 * @param { String } origin 源路径
		 * @param { String } target 目标路径
		 * **/
		const copyDirectory = (origin, target, cover = true, filterCb) => {
		    if (cover)
		        removeFile(target);
		    ...
		    for (const entry of entries) {
		        ...
		        const destPath = path.resolve(target, entry.name);
		...
		/**
		 * 以当前工作目录为基准，查看一个文件是否存在
		 * @param { String } name 文件名称
		 * @returns { Boolean }
		 * **/
		const cwdExistsFile = (name) => {
		 ...
		 * */
		const newDirectory = (dirPath) => {
		    return new Promise((resolve) => {
		        if (fs.existsSync(dirPath))
		            return resolve(0);
		        fs.mkdir(dirPath, { recursive: true }, () => resolve(1));
		    });
		};
		/**
		 ...
		 * @param { string | NodeJS.ArrayBufferView } content 写入的内容
		 * */
		const writeFile = (filename, content) => {
		    return new Promise((resolve, reject) => {
		        fs.writeFile(filename, content, { flag: 'a+', encoding: 'utf-8' }, (error) => {
		...
		const traverseDirectory = (dirPath) => {
		    return new Promise(async (resolve) => {
		        try {
		            ...
		            const files = [];
		            const ignoreFilenames = process.platform === 'darwin'
		                ? ['.DS_Store', '._.DS_Store']
		                : [];
		            for (const filename of filenames) {
		                if (ignoreFilenames.indexOf(filename) !== -1)
		                    continue;
		...
		const isLinkFile = (path) => {
		    return new Promise((resolve) => {
		        fs.readlink(path, (err, linkString) => {
		            ...
		            console.info(`${path} is link file. target is: ${linkString}`);
		            resolve(true);
		        });
		    });
		};
		 ...
		 * **/
		const checkXPermission = (filepath) => {
		    return new Promise((resolve, reject) => {
		        fs.access(filepath, fs.constants.X_OK, (error) => {
		            if (!error)
		                return resolve();
		            fs.chmod(filepath, fs.constants.S_IXUSR, (error) => {
		                error ? reject(error) : resolve();
		            });
		        });
		    });
		 ...
		 * **/
		const deleteFolder = (dirPath, whiteList = []) => {
		    if (fs.existsSync(dirPath)) {
		        const files = fs.readdirSync(dirPath);
		        for (const file of files) {
		            const filePath = path.join(dirPath, file);
		            if (fs.lstatSync(filePath).isDirectory()) {
		                deleteFolder(filePath, whiteList);
		            }
		            else {
		 ...
		 * @param { Object } options 执行命令的参数
		 * **/
		const execCommandPro = (command, options) => {
		    return new Promise((resolve, reject) => {
		        const commandList = command.split(' ');
		        const spawnOption = Object.assign({
		            shell: true,
		            encoding: 'utf8'
		        }, options);
		        if (spawnOption.cwd && !fs.existsSync(spawnOption.cwd)) {
		            throw new Error('exec command cwd is not exists...');
		        }
		...
		/**
		 * 执行安装的命令
		 * @param { String } command 命令符
		 * @param { Array<String> } args 参数数组
		 * **/
		const execCommand = (command, ...args) => {
		    return new Promise((resolve, reject) => {
		        const installProcess = node_child_process.spawnSync(command, args, {
		            shell: true,
		            stdio: 'inherit',
		            encoding: 'utf-8'
		        });
		        if (installProcess.error)
		            reject(installProcess.error);
		        else
		            resolve(installProcess);
		    });
		};
		/**
		 * 执行命令，获取执行的命令结果
		 * **/
		const getExecCommandResult = (command, options) => {
		    return new Promise((resolve) => {
		        ...
		        const commandList = command.split(' ');
		        const result = node_child_process.spawnSync(commandList[0], commandList.slice(1), args);
		        if (result.stderr)
		 ...
		 * **/
		const referToAppData = (refer) => {
		    return path.resolve(appData(), refer || '');
		};
		/**
		...
		const getPidByName = (name) => {
		    // 执行不同的命令获取进程信息
		    const isWindows = systemInfo().isWindow;
		    const command = isWindows ? 'tasklist' : 'ps -ef';
		    const pidList = [];
		    return new Promise((resolve) => {
		        node_child_process.exec(command, (err, stdout) => {
		            if (err) {
		            ...
		            // 将命令输出按行分割成数组
		            const lines = stdout.split(isWindows ? '\r\n' : '\n');
		            // 遍历每一行输出
		            for (let i = 0; i < lines.length; i++) {
		                // 如果进程名中包含指定的字符串，说明找到了目标进程
		 ...
		 * **/
		const isActiveProcess = (processName) => {
		    return new Promise((resolve) => {
		        getPidByName(processName).then((list) => resolve(list));
		    });
		};
		/**
		 * 根据进程的PID，结束此进程
		...
		const killProcessPid = (pid) => {
		    ...
		    if (!Array.isArray(pid))
		        pid = [pid];
		    const tasks = [], isWindows = systemInfo().isWindow;
		    function killTak(id) {
		...
		/**
		 * 检查包版本管理器的合法性
		 * @param { string } name 包管理器的名称
		 * **/
		const invalidPackageCli = (name) => {
		    return packageMangerViewer.has(name);
		};
		/**
		 * 获取当前项目的包版本管理器, 目前支持 yarn|npm
		 * @param { string } targetPath 目标路径
		 * **/
		const getPackageMangerName = (targetPath = process.cwd()) => {
		    const typeFileNameExistsCli = (filename) => fs.existsSync(path.resolve(targetPath, filename));
		    for (const [key, value] of packageMangerViewer) {
		        if (typeFileNameExistsCli(value)) {
		            return key;
		        }
		    }
		};
		...
		exports.createCoverDir = createCoverDir;
		exports.createNotExistsFolder = createNotExistsFolder;
		exports.cwdExistsFile = cwdExistsFile;
		exports.debounce = debounce;
		exports.deleteFolder = deleteFolder;
		exports.differenceArrayList = differenceArrayList;
		exports.execCommand = execCommand;
		exports.execCommandPro = execCommandPro;
		exports.existsFileForCwd = existsFileForCwd;
		exports.flatObject = flatObject;
		exports.getExecCommandResult = getExecCommandResult;
		exports.getPackageMangerName = getPackageMangerName;
		exports.getPidByName = getPidByName;
		exports.handlerJsonContent = handlerJsonContent;
		exports.homeDir = homeDir;
		exports.intersectionArrayList = intersectionArrayList;
		exports.invalidPackageCli = invalidPackageCli;
		exports.isActiveProcess = isActiveProcess;
		exports.isArray = isArray;
		exports.isEmptyObject = isEmptyObject;
		exports.isLinkFile = isLinkFile;
		...
		exports.readDirPathTypeFile = readDirPathTypeFile;
		exports.readFileIsExists = readFileIsExists;
		exports.referToAppData = referToAppData;
		...
		exports.writeCoverFile = writeCoverFile;
		exports.writeFile = writeFile;
		exports.writeNotExistsFile = writeNotExistsFile; 
	} (common));
	return common;
}
...
var dfsParser;
var hasRequiredDfsParser;

function requireDfsParser () {
	if (hasRequiredDfsParser) return dfsParser;
	hasRequiredDfsParser = 1;
	dfsParser = function (dict, result) {
...
var fileAction;
var hasRequiredFileAction;

function requireFileAction () {
	if (hasRequiredFileAction) return fileAction;
	hasRequiredFileAction = 1;
	const fs = require$$0$3;
	const path = require$$1$2;
...
function requireLib$3 () {
	...
	const dfsParser = requireDfsParser();
	const basicCommon = requireCommon();
	const fileAction = requireFileAction();
...
var constance;
var hasRequiredConstance;

function requireConstance () {
	if (hasRequiredConstance) return constance;
	hasRequiredConstance = 1;
	constance = {
	    SUPPORT_SYSTEM: ["win32"], // 支持的系统平台
	    NODE_MINI_VERSION: "18.0.0", // 最低的Node版本
	    APP_DIR_NAME: ".mvanner", // APP缓存目录名称
	    SOURCE_FILE_NAME: ".mv.cli.rc", // APP配置文件名称
	    INIT_PROJECT_DEFAULT_NAME: "mvanner-demo", // 默认的初始化项目名称
	    INIT_PROJECT_VITE_REMOTE: "https://gitee.com/memory_s/mv-template.git", // 初始化项目的vite管理仓库地址
...
function requireHookEngine () {
	...
	const node_async_hooks_1 = require$$0$4;
...
function requireCjs$d () {
	if (hasRequiredCjs$d) return cjs$a;
	hasRequiredCjs$d = 1;
	(function (exports) {
		var __importDefault = (cjs$a && cjs$a.__importDefault) || function (mod) {
		...
		const node_process_1 = __importDefault(require$$0$6);
...
function requireTheme () {
	...
	const figures_1 = __importDefault(/*@__PURE__*/ requireCjs$d());
	theme.defaultTheme = {
	    prefix: {
...
function requireLines () {
	...
	lines.lines = lines$1;
	const utils_mjs_1 = /*@__PURE__*/ requireUtils();
...
function requireCjs$c () {
	if (hasRequiredCjs$c) return cjs$9;
	hasRequiredCjs$c = 1;
	(function (exports) {
		var _a;
		Object.defineProperty(exports, "__esModule", { value: true });
...
function requireCreatePrompt () {
	...
	const readline = __importStar(require$$0$8);
	const node_async_hooks_1 = require$$0$4;
...
function requireCjs$b () {
	...
	hasRequiredCjs$b = 1;
	(function (exports) {
		var __createBinding = (cjs$b && cjs$b.__createBinding) || (Object.create ? (function(o, m, k, k2) {
		    if (k2 === undefined) k2 = k;
		    var desc = Object.getOwnPropertyDescriptor(m, k);
		    if (!desc || ("get" in desc ? !m.__esModule : desc.writable || desc.configurable)) {
		      desc = { enumerable: true, get: function() { return m[k]; } };
		...
		var __exportStar = (cjs$b && cjs$b.__exportStar) || function(m, exports) {
...
function requireCjs$a () {
	if (hasRequiredCjs$a) return cjs$c;
	hasRequiredCjs$a = 1;
	(function (exports) {
		var __awaiter = (cjs$c && cjs$c.__awaiter) || function (thisArg, _arguments, P, generator) {
		...
		var __importDefault = (cjs$c && cjs$c.__importDefault) || function (mod) {
		    return (mod && mod.__esModule) ? mod : { "default": mod };
		};
		Object.defineProperty(exports, "__esModule", { value: true });
		...
		var core_2 = /*@__PURE__*/ requireCjs$b();
...
function requireCjs$9 () {
	...
	hasRequiredCjs$9 = 1;
	var __awaiter = (cjs$8 && cjs$8.__awaiter) || function (thisArg, _arguments, P, generator) {
	    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
	    return new (P || (P = Promise))(function (resolve, reject) {
	        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
	        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
...
function requireCjs$8 () {
	...
	const core_1 = /*@__PURE__*/ requireCjs$b();
	cjs$7.default = (0, core_1.createPrompt)((config, done) => {
...
function requireCjs$7 () {
	if (hasRequiredCjs$7) return cjs$6;
	hasRequiredCjs$7 = 1;
	var __awaiter = (cjs$6 && cjs$6.__awaiter) || function (thisArg, _arguments, P, generator) {
	    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
	    return new (P || (P = Promise))(function (resolve, reject) {
	        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
	        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
	...
	const core_1 = /*@__PURE__*/ requireCjs$b();
	cjs$6.default = (0, core_1.createPrompt)((config, done) => {
...
function requireCjs$6 () {
	if (hasRequiredCjs$6) return cjs$5;
	hasRequiredCjs$6 = 1;
	var __awaiter = (cjs$5 && cjs$5.__awaiter) || function (thisArg, _arguments, P, generator) {
	    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
	    return new (P || (P = Promise))(function (resolve, reject) {
	        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
	        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
	...
	cjs$5.default = (0, core_1.createPrompt)((config, done) => {
	    var _a;
...
function requireCjs$5 () {
	if (hasRequiredCjs$5) return cjs$4;
	hasRequiredCjs$5 = 1;
	(function (exports) {
		var __importDefault = (cjs$4 && cjs$4.__importDefault) || function (mod) {
		    return (mod && mod.__esModule) ? mod : { "default": mod };
		};
		Object.defineProperty(exports, "__esModule", { value: true });
		exports.Separator = void 0;
...
function requireCjs$4 () {
	if (hasRequiredCjs$4) return cjs$3;
	hasRequiredCjs$4 = 1;
	(function (exports) {
...
function requireCjs$3 () {
	...
	hasRequiredCjs$3 = 1;
	var __awaiter = (cjs$2 && cjs$2.__awaiter) || function (thisArg, _arguments, P, generator) {
	    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
	    return new (P || (P = Promise))(function (resolve, reject) {
	        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
	        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
...
function requireCjs$2 () {
	...
	hasRequiredCjs$2 = 1;
	(function (exports) {
		var __awaiter = (cjs$1 && cjs$1.__awaiter) || function (thisArg, _arguments, P, generator) {
		    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
		    return new (P || (P = Promise))(function (resolve, reject) {
		        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
		        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
...
function requireCjs () {
	if (hasRequiredCjs) return cjs$d;
	hasRequiredCjs = 1;
	(function (exports) {
		var __importDefault = (cjs$d && cjs$d.__importDefault) || function (mod) {
		    return (mod && mod.__esModule) ? mod : { "default": mod };
		};
		Object.defineProperty(exports, "__esModule", { value: true });
		...
		var checkbox_1 = /*@__PURE__*/ requireCjs$a();
...
function requireGitStorage () {
	...
	const { basicCommon, fileAction } = requireLib$3();
...
function requireInit () {
	...
	const { basicCommon, platform, dfsParser } = requireLib$3();
	const {
...
var commandConfig;
var hasRequiredCommandConfig;

function requireCommandConfig () {
	if (hasRequiredCommandConfig) return commandConfig;
	hasRequiredCommandConfig = 1;
...
function requireRun () {
	if (hasRequiredRun) return Run_1;
	hasRequiredRun = 1;
	const fs = require$$0$3;
	const path = require$$1$2;
	const { Inquirer } = requireModules();
...
function requireCommand () {
	...
	class BaseCommand extends RegisterCommandUtil {
	    constructor() {
	        super({
	            commandOption: requireCommand$1(),
	        });
	    }
	    start() {
...
function requireLib () {
	...
	hasRequiredLib = 1;
	const { name, version } = require$$0$1;
